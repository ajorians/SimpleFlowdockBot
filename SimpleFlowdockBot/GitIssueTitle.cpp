...
GitIssueTitleHandler::GitIssueTitleHandler(const std::string& strGithubToken)
   : m_pCurl( NULL ), m_pCookies( NULL ), m_strGithubToken( strGithubToken )
{
   //curl_global_init(CURL_GLOBAL_ALL);
   m_pCurl = curl_easy_init();

   curl_easy_setopt( m_pCurl, CURLOPT_WRITEFUNCTION, GitIssueTitleHandler::write_callback );
...
GitIssueTitleHandler::~GitIssueTitleHandler()
{
...
bool GitIssueTitleHandler::HasIssue( const std::string& strMessage )
{
   return GitIssueTitleHandler::IssuesFromMessage( strMessage ).size() > 0;
}

std::vector<std::string> GitIssueTitleHandler::IssuesFromMessage( const std::string& strMessage )
{
   ...
   while ( true )
   {
      ...
      if ( nEnd >= (int)strMessage.length() || strMessage[nEnd] != 'i' )//issues/
         continue;
      ...
      std::string strURL = "https://api.github.com/repos/TechSmith/" + strRepo + "/issues/" + strNumber;
...
std::pair<std::string, std::vector<std::string> > GitIssueTitleHandler::GetIssueTitleAndLabels(const std::string& strURL)
{
   std::string strAddress = strURL;

   std::pair<std::string, std::vector<std::string> > pairRet;
   ...
   //Get the Label names:
   {
      int nStart = m_strWrite.find("\"labels\":");
      if( nStart == std::string::npos )
         return pairRet;

      nStart = nStart + strlen("\"labels\":");
      ...
      std::string strLabels = m_strWrite.substr(nStart, nEnd-nStart);

      int nStartLabelName = 0;
      std::vector<std::string> arrstrLabels;
      while ( true )
      {
         nStartLabelName = strLabels.find( "\"name\":", nStartLabelName );
         if ( nStartLabelName == std::string::npos )
            break;

         nStartLabelName = nStartLabelName + strlen( "\"name\":" );

         nStartLabelName = strLabels.find( "\"", nStartLabelName );
         if ( nStartLabelName == std::string::npos )
            break;

         nStartLabelName += strlen( "\"" );

         int nEndLabelName = strLabels.find( "\"", nStartLabelName );
         if ( nEndLabelName == std::string::npos || nEndLabelName <= nStartLabelName )
            break;

         std::string strLabel = strLabels.substr( nStartLabelName, nEndLabelName - nStartLabelName );
         arrstrLabels.push_back( strLabel );

         nStartLabelName = nEndLabelName + 1;
      }

      pairRet.second = arrstrLabels;
   }
